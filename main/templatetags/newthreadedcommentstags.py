...
from main.forms import SlimThreadedCommentForm, SlimFreeThreadedCommentForm
from main.models import UpdatedFreeThreadedComment, UpdatedThreadedComment
...
def get_comment_url(content_object, parent=None):
    ...
    creation of new ``UpdatedThreadedComment`` objects.
    """
    kwargs = get_contenttype_kwargs(content_object)
    if parent:
        if not isinstance(parent, UpdatedThreadedComment):
            raise template.TemplateSyntaxError, "get_comment_url requires its parent object to be of type UpdatedThreadedComment"
...
def get_comment_url_ajax(content_object, parent=None, ajax_type='json'):
    """
    Given an object and an optional parent, this tag gets the URL to POST to for the
    creation of new ``UpdatedThreadedComment`` objects.  It returns the latest created object
    ...
    if parent:
        if not isinstance(parent, UpdatedThreadedComment):
            raise template.TemplateSyntaxError, "get_comment_url_ajax requires its parent object to be of type UpdatedThreadedComment"
        kwargs.update({'parent_id' : getattr(parent, 'pk', getattr(parent, 'id'))})
...
def get_comment_url_json(content_object, parent=None):
    ...
    try:
        ...
        raise template.TemplateSyntaxError, "get_comment_url_json requires its parent object to be of type UpdatedThreadedComment"
    return ''
...
def get_comment_url_xml(content_object, parent=None):
    ...
    try:
        ...
        raise template.TemplateSyntaxError, "get_comment_url_xml requires its parent object to be of type UpdatedThreadedComment"
    return ''

def get_free_comment_url(content_object, parent=None):
    """
    Given an object and an optional parent, this tag gets the URL to POST to for the
    creation of new ``UpdatedFreeThreadedComment`` objects.
    """
    kwargs = get_contenttype_kwargs(content_object)
    if parent:
        if not isinstance(parent, UpdatedFreeThreadedComment):
            raise template.TemplateSyntaxError, "get_free_comment_url requires its parent object to be of type UpdatedFreeThreadedComment"
        kwargs.update({'parent_id' : getattr(parent, 'pk', getattr(parent, 'id'))})
...
def get_free_comment_url_ajax(content_object, parent=None, ajax_type='json'):
    """
    Given an object and an optional parent, this tag gets the URL to POST to for the
    creation of new ``UpdatedFreeThreadedComment`` objects.  It returns the latest created object
    in the AJAX form of the user's choosing (json or xml).
    """
    kwargs = get_contenttype_kwargs(content_object)
    kwargs.update({'ajax' : ajax_type})
    if parent:
        if not isinstance(parent, UpdatedFreeThreadedComment):
            raise template.TemplateSyntaxError, "get_free_comment_url_ajax requires its parent object to be of type UpdatedFreeThreadedComment"
        kwargs.update({'parent_id' : getattr(parent, 'pk', getattr(parent, 'id'))})
...
def get_free_comment_url_json(content_object, parent=None):
    ...
    try:
        ...
        raise template.TemplateSyntaxError, "get_free_comment_url_json requires its parent object to be of type UpdatedFreeThreadedComment"
    return ''
...
def get_free_comment_url_xml(content_object, parent=None):
    ...
    try:
        ...
        raise template.TemplateSyntaxError, "get_free_comment_url_xml requires its parent object to be of type UpdatedFreeThreadedComment"
    return ''

def auto_transform_markup(comment):
    """
    Given a comment (``UpdatedThreadedComment`` or ``UpdatedFreeThreadedComment``), this tag
...
def do_get_threaded_comment_tree(parser, token):
    ...
    additional ``depth`` integer attribute annotated onto each ``UpdatedThreadedComment``.
    """
...
def do_get_free_threaded_comment_tree(parser, token):
    ...
    additional ``depth`` integer attribute annotated onto each ``UpdatedFreeThreadedComment.``
    """
...
class CommentTreeNode(template.Node):
    ...
    def render(self, context):
        ...
        context[self.context_name] = UpdatedThreadedComment.public.get_tree(content_object, root=tree_root)
        return ''
        ...
        try:
            ...
            if self.tree_root_str == 'as':
                ...
                try:
                    ...
                    tree_root = self.tree_root_str
        context[self.context_name] = UpdatedFreeThreadedComment.public.get_tree(content_object, root=tree_root)
        return ''

def do_get_comment_count(parser, token):
    """
    Gets a count of how many UpdatedThreadedComment objects are attached to the given
    object.
    """
    ...
    if split[1] != 'for' or split[3] != 'as':
        raise template.TemplateSyntaxError, error_message
    return UpdatedThreadedCommentCountNode(split[2], split[4])

class UpdatedThreadedCommentCountNode(template.Node):
    def __init__(self, content_object, context_name):
        self.content_object = template.Variable(content_object)
        self.context_name = context_name
    def render(self, context):
        content_object = self.content_object.resolve(context)
        context[self.context_name] = UpdatedThreadedComment.public.all_for_object(content_object).count()
        return ''
        
def do_get_free_comment_count(parser, token):
    """
    Gets a count of how many UpdatedFreeThreadedComment objects are attached to the 
    given object.
    """
    error_message = "%r tag must be of format {%% %r for OBJECT as CONTEXT_VARIABLE %%}" % (token.contents.split()[0], token.contents.split()[0])
    try:
    ...
    if split[1] != 'for' or split[3] != 'as':
        raise template.TemplateSyntaxError, error_message
    return UpdatedFreeThreadedCommentCountNode(split[2], split[4])

class UpdatedFreeThreadedCommentCountNode(template.Node):
    def __init__(self, content_object, context_name):
        self.content_object = template.Variable(content_object)
        self.context_name = context_name
    def render(self, context):
        content_object = self.content_object.resolve(context)
        context[self.context_name] = UpdatedFreeThreadedComment.public.all_for_object(content_object).count()
        return ''
...
def do_get_threaded_comment_form(parser, token):
    """
    Gets a UpdatedFreeThreadedCommentForm and inserts it into the context.
    """
    ...
    if "free" in split[0]:
        ...
        is_free = False
    return UpdatedThreadedCommentFormNode(split[2], free=is_free)

class UpdatedThreadedCommentFormNode(template.Node):
    def __init__(self, context_name, free=False):
        self.context_name = context_name
        self.free = free
    def render(self, context):
        if self.free:
            form = SlimFreeThreadedCommentForm()
        else:
            form = SlimThreadedCommentForm()
...
class LatestCommentsNode(template.Node):
    ...
    def render(self, context):
        if self.free:
            comments = UpdatedFreeThreadedComment.objects.order_by('-date_submitted')[:self.num]
        else:
            comments = UpdatedThreadedComment.objects.order_by('-date_submitted')[:self.num]
